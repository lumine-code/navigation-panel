...
module.exports = {
  activate() {
    ...
    // `navigation.headers` service, connected directly rather than through
    // the service hub — the layer sees exactly what an external consumer
    // would.
    markerLayer.activate();
    this.markerLayerConnection = markerLayer.connect(this.provideNavigationHeaders());
  },
  ...
  getNavigationTree() {
    if (!this.navigationTree) {
      ...
      tree.onDidDestroy(() => {
        if (this.navigationTree === tree) {
          this.navigationTree = null;
        }
      });
      tree.update(this.headers, { instant: true });
    }
