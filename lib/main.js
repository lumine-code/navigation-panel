...
module.exports = {
  provideBackgroundTips() {
    return {
      packageName: "navigation-panel",
      tips: [
        "You can jump between the sections of the current document with {{ 'navigation-panel:toggle-focus' | keystroke }}",
      ],
    };
  },
  ...
  activate() {
    ...
    // Keep the public list model available to consumers after activation, but
    // construct its select-list host outside the measured synchronous hook.
    // The model is still ready before the next user event (and before an
    // activation promise continuation runs).
    queueMicrotask(() => {
    ...
    // Workspace restoration and pane-copy semantics still get a stable tree
    // identity from the microtask above; deserializers that run before
    // activation use getNavigationTree() directly.
  ...
  consumeNavigationAdapter(adapter) {
    ...
    // Service consumers are wired while the initial package batch is still on
    // the activation stack. Keep that bootstrap cheap: the adapter registry is
    // enough to publish the service, and the first-use hook below will build
    // the scanner/marker runtime after the batch has completed. If the panel
    // is already live, consume and subscribe immediately as before.
    if (this._runtimeReady) {
      const registration = this.adapterManager.consume(adapter);
      const item = lumine.workspace.getCenter().getActivePaneItem();
      if (item && adapter.handlesItem?.(item)) this.adapterSubscribe(adapter, item);
      return registration;
    }
    const main = this;
    queueMicrotask(() => {
      if (!main.navigationAdapters?.has(adapter)) return;
      if (!main._runtimeReady) ensurePanelRuntime(main);
      if (!main.navigationAdapters.has(adapter)) return;
      const item = lumine.workspace.getCenter().getActivePaneItem();
      if (item && adapter.handlesItem?.(item)) main.adapterSubscribe(adapter, item);
    });
    return new Disposable(() => {
      main.navigationAdapters?.delete(adapter);
      if (main._runtimeReady) main.adapterManager.registry.delete(adapter);
    });
  },
